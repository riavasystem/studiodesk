...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-svh flex-1 flex-col overflow-hidden bg-background">
      <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-3 py-2 sm:px-6 md:px-10">
        <Link href="/dashboard" className="font-mono text-sm tracking-[0.3em] text-white/80 uppercase">
          StudioDesk
        </Link>
        <div className="flex items-center gap-2 sm:gap-4">
          <span className="hidden text-sm text-white/60 sm:inline">{user?.full_name}</span>
      ...
      <main className="flex-1 overflow-y-auto px-3 py-2 sm:px-6 md:px-10">{children}</main>
    </div>
  );
}
